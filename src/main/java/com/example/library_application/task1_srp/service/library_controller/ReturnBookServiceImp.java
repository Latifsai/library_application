...
import com.example.library_application.task1_srp.dto.readerBook.ReturnBookDTOResponse;
...
public class ReturnBookServiceImp implements AllService<ReturnBookDTOResponse, TakeAndReturnBookDTORequest> {
    ...
    public ReturnBookDTOResponse execute(TakeAndReturnBookDTORequest request) {
        ...
        var controller = convertor.convertByReturningBookRequestsToController(request, book.get(), reader.get());
        libraryControllerRepository.save(controller);

        return new ReturnBookDTOResponse(book.get().getTitle(), reader.get().getForename(), reader.get().getSurname(), controller.getBookReturnDate());
    }
}
